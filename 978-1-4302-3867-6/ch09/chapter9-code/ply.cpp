...
void loadPLYFile(const char* filename, pcl::PointCloud<pcl::PointNormal>& cloud)
{
    ...
    cloud.points.resize(ply_vertices.size());

    for (int i = 0; i < ply_vertices.size(); ++i)
    {
        const PlyVertex& v = ply_vertices[i];
        cloud.points[i].getVector3fMap() = Eigen::Vector3f(v.x, v.y, v.z);
        cloud.points[i].getNormalVector3fMap() = Eigen::Vector3f(v.nx, v.ny, v.nz);
    }
}
...
void binary_get_element(PlyFile *plyfile, char *elem_ptr)
{
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list == PLY_LIST) {          /* list */
      ...
      int len;
      char *str;
      assert(fread (&len, sizeof(int), 1, fp));
      str = (char *) myalloc (len);
      assert(fread (str, len, 1, fp));
      if (store_it) {
	char **str_ptr;
        item = elem_data + prop->offset;
	str_ptr = (char **) item;
